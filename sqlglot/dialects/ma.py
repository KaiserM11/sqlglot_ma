        ...
        return exp_class(
            ...
            format=exp.Literal.string(
                format_time(
                    args[0].name.lower(),
                    (
                        {**MA.TIME_MAPPING, **FULL_FORMAT_TIME_MAPPING}
                        if full_format_mapping
                        else MA.TIME_MAPPING
                    ),
                )
            ),
        )
...
def _parse_format(args: t.List) -> exp.Expression:
    ...
    if fmt:
        fmt = exp.Literal.string(
            format_time(fmt.name, MA.FORMAT_TIME_MAPPING)
            if len(fmt.name) == 1
            else format_time(fmt.name, MA.TIME_MAPPING)
        )
...
def _format_sql(self: MA.Generator, expression: exp.NumberToStr | exp.TimeToStr) -> str:
    fmt = expression.args["format"]

    if not isinstance(expression, exp.NumberToStr):
        if fmt.is_string:
            mapped_fmt = format_time(fmt.name, MA.INVERSE_TIME_MAPPING)
...
def _string_agg_sql(self: MA.Generator, expression: exp.GroupConcat) -> str:
...
def _json_extract_sql(
    self: MA.Generator, expression: exp.JSONExtract | exp.JSONExtractScalar
) -> str:
    ...
    class Parser(parser.Parser):
        ...
        def _parse_convert(
            ...
            if self._match(TokenType.COMMA):
                ...
                if format_val_name not in MA.CONVERT_FORMAT_MAPPING:
                    raise ValueError(
                        f"CONVERT function at T-SQL does not support format style {format_val_name}"
                    )

                format_norm = exp.Literal.string(MA.CONVERT_FORMAT_MAPPING[format_val_name])
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {
            **generator.Generator.TRANSFORMS,
            ...
            exp.TimeToStr: rename_func("DATETIME_INTERVAL"),
